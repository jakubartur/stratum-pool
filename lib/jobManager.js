...
var JobManager = module.exports = function JobManager(options) {
  ...
  var blockHasher = (function () {
    switch (options.coin.algorithm) {
      ...
      case 'kawpow':
        return function (d, nTimeInt) {
          return util.reverseBuffer(util.kawpowHash(d, nTimeInt));
        };
      default:
        return function (d) {
  ...
  this.processTemplate = function (rpcData) {
    ...
    // Block is new if A) it's the first block we have seen so far or B) the blockhash is different and the
  ...
  this.processShare = function (miner_given_jobId, previousDifficulty, difficulty, miner_given_nonce, ipAddress, port, workerName, miner_given_header, miner_given_mixhash, extraNonce1, callback_parent) {
    ...
    var zeroPad = '0'.repeat(64 - adjPow.toString(16).length);
